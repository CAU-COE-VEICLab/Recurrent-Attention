...
from networks.nnFormer.MemorynnFormer_seg import MemorynnFormer
...
elif args.network == 'MemorySwinUNETR':
    model = MemorySwinUNETR(
        img_size=(96, 96, 96),
        in_channels=1,
...
elif args.network == 'MemorynnFormer':
    model = MemorynnFormer(input_channels=1, num_classes=out_classes).to(device)
