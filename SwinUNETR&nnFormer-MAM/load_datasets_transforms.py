...
def infer_post_transforms_labels(args, test_transforms, out_classes):

    post_transforms = Compose([
        EnsureTyped(keys="label"),
        # Activationsd(keys="label", softmax=True),
        Invertd(
            keys="label",  # invert the `pred` data field, also support multiple fields
            transform=test_transforms,
            orig_keys="image",  # get the previously applied pre_transforms information on the `img` data field,
            # then invert `pred` based on this information. we can use same info
            # for multiple fields, also support different orig_keys for different fields
        ...
        ## If monai version <= 0.6.0:
        AsDiscreted(keys="label", n_classes=out_classes),
        ## If moani version > 0.6.0:
        # AsDiscreted(keys="pred", argmax=True)
        # KeepLargestConnectedComponentd(keys='pred', applied_labels=[1, 3]),
        SaveImaged(keys="label", meta_keys="pred_meta_dict", output_dir=args.output,
                   output_postfix="label", output_ext=".nii.gz", resample=True),
    ])
