...
Recurrent-Attention
@author: Guorun Li
Our code is based on 3DUX-Net, if you need to use it, please be sure to reference us and 3DUX-Net!
"""
...
from networks.nnFormer.nnFormerRecA_seg import nnFormerRecA
from networks.SwinUnetr.swin_unetr import SwinUNETRRecA
...
parser = argparse.ArgumentParser(description='Recurrent-Attention models inference hyperparameters for medical image segmentation')
...
parser.add_argument('--network', type=str, default='MemorySwinUNETR', required=True, help='Network models: {MemorySwinUNETR, MemorynnFormer, TransBTS, nnFormer, UNETR, SwinUNETR, 3DUXNET}')
...
elif args.network == 'SwinUNETRRecA':
    model = SwinUNETRRecA(
        img_size=(96, 96, 96),
        in_channels=1,
...
elif args.network == 'nnFormerRecA':
    model = nnFormerRecA(input_channels=1, num_classes=out_classes).to(device)
